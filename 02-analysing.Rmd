...
# Load the package, and set a global random-number seed for the reproducible
...
```{r, fig=TRUE, include=TRUE, fig.width=8.5, fig.height=5, fig.cap="Plots of species mean reflectance curves with corresponding relative usml cone stimulations (insets)."}
...
```{r}
vismod.idi <- vismodel(sppspec, visual = ideal_dichromat, relative = FALSE)
vismod.idi
```
...
```{r, fig=TRUE, include=TRUE, fig.width=4, fig.height=4, fig.cap="Idealized reflectance spectra and their projection on the axes of segment classification"}
fakedata1 <- vapply(
  seq(100, 500, by = 20),
  function(x) rowSums(cbind(
      dnorm(300:700, x, 30),
...
vis.flowers <- vismodel(
  flowers, visual = "apis", qcatch = "Ei", relative = FALSE, vonkries = TRUE, 
  achromatic = "l", bkg = "green"
)
hex.flowers <- colspace(vis.flowers, space = "hexagon")
...
fakemantisshrimp <- sensmodel(c(325, 350, 400, 425, 450, 500, 550, 600, 650, 700), 
...
adjacent(butterflies_class[[2]], xscale = 200, xpts = 200, bkgID = 1,
...
```{r, fig=TRUE, fig.cap="A cryptic lizard, along with a possible sampling grid for spectral measurement.", fig.show="hold", out.width="65%"}
...
plot(lizard)
points(expand.grid(seq(0, dim(lizard)[1], 25), seq(0, dim(lizard)[1], 25)),
       pch = 16, col = 'red')
```
...
library(NbClust)
clust <- NbClust(jnd.fakescene, diss = jnd.mat, distance = NULL, 
                 method = 'centroid', index = 'hartigan')
clust$Best.nc
```
