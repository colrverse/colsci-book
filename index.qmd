...
```{r}
#| echo: false
#| fig-cap: A non-exhaustive overview of the colour-pattern analysis workflow in pavo,
#|   displaying some key functions at each stage.
#| out-width: 100%
#| fig-dpi: 72
knitr::include_graphics("fig/workflow.png")
```

In the remaining chapters we begin by detailing the importing, processing and visualisation of spectral and image data, before moving on to discussion of the many analyses that `pavo` allows. Our hope is to demonstrate the flexibility of `pavo`, and to provide a cohesive, reproducible workflow for colour pattern analysis within `R`. As always, the development version of `pavo` can be found on {{< var links.pavo_repo >}}, while the stable release is available via CRAN.

This handbook describes `pavo` 2.10.0 and later. Several of the arguments used in the following chapters do not exist in earlier versions, so if an example returns an "unused argument" error, updating the package is the first thing to try. The output shown throughout was generated with version `r packageVersion("pavo")`.
...
For suggestions, assistance and/or bug reports, we suggest getting in touch via the [colRverse discussion board](https://github.com/colrverse/colRverse/discussions). It requires a github account, however, so if you don't have and/or want one the feel free to just [email Tom](mailto:thomas.white026@gmail.com) and we'll get back to you as soon as possible. If you have a bug to report, we'd appreciate it if you could also include a reproducible example when possible. Users familiar with GitHub may prefer to open an issue on the project's {{< var links.pavo_repo >}}, or to make a pull-request directly.
