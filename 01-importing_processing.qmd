...
```{r}
#| label: explorespecfig
#| fig: true
#| include: true
#| fig-width: 6
#| fig-height: 4.5
#| fig-cap: Result from `explorespec()`, showing the three measurements for each individual
#|   cardinal in separate panels
...
```{r}
#| label: exploresppmeans
#| fig: true
#| include: true
#| fig-width: 5
#| fig-height: 3.5
#| fig-cap: Result from `explorespec()` for species means
...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: Result for raw (grey line) and smoothed (red line) reflectance data for the
#|   parakeet
...
#| fig: true
#| include: true
#| fig-cap: Results for min (left), max (centre), and both normalisations (right)
#| layout-ncol: 2
#| fig-align: center
# Plot results
...
A different sort of processing is available through `opt = "clip"`, which removes a range of wavelengths and fills the gap by linear interpolation between the values either side of it. This is for regions that are unusable for reasons smoothing will not fix, such as a filter cut-off, a saturated detector, or a second-order artefact. The range is given through `clip_range`, and a list of ranges can be passed if more than one region needs removing. Where both are requested, clipping is applied before smoothing.
...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: A single spectrum before (black) and after (red) clipping 400-450 nm, with
#|   the removed region replaced by linear interpolation.
spec.clip <- procspec(sppspec, opt = "clip", clip_range = c(400, 450))

plot(sppspec[, 5] ~ sppspec$wl,
     type = "l",
     xlab = "Wavelength (nm)",
     ylab = "Reflectance (%)")
lines(spec.clip[, 5] ~ spec.clip$wl, col = "red")
```
...
#| fig: true
#| include: true
#| fig-cap: Plot of PC1 loading versus wavelength (left) and species mean spectra sorted
#|   vertically from lowest to highest PC1 value (right; values on right hand axis are
#|   column identities).
#| layout-ncol: 2
...
#| fig-width: 6
#| fig-height: 4.5
#| fig-cap: Plots showing original reflectance curve including negative values (left)
#|   and two processed curves using `fixneg = addmin` (center) and `fixneg = zero` (right).
#| layout-ncol: 2
# Plot it
...
```{r}
#| echo: false
#| fig-cap: Anas carolinensis, by pixabay user Cock-Robin, CC0
#| out-width: 100%
...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: Overlay plot of the teal angle-dependent reflectance with colours of each
#|   curve being an approximation of the perceived colour.
data(teal)
...
#| fig: true
#| include: true
#| fig-cap: Stacked plot of the raw (left) and normalized (right) teal angle-dependent
#|   reflectance
#| layout-ncol: 2
...
Labels identifying each spectrum can be added along the right-hand margin by passing `labels = TRUE`, which works for overlay and stacked plots alike. They default to the column names of the object, `labels.stack` substitutes a vector of your own, and `labels.cex` sets their size. The right margin usually needs widening through `par()` to make room for them.
...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: Stacked teal spectra with each curve labelled by its incident angle.
oldpar <- par(mar = c(4, 4, 2, 4))

plot(teal, type = "s", col = spec2rgb(teal), labels = TRUE, labels.cex = 0.7)

par(oldpar)
```

Spectra can also be identified and subset by index, using the `select` argument in `plot.rspec()`. Note that the first index is actually 2, preserving the sequence in the original dataset (since the first column is wavelength). Though this may seem confusing at first ("why is my first spec number 2?") this preserves subsetting hierarchy: using `plot(teal, select = 2)` will show the same spectra that would be selected if you use `teal[ ,2]`.
...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: Heatmap plot for angle-resolved reflectance measurements of the green-winged
#|   teal.
...
#| fig-height: 6
#| layout-ncol: 2
#| fig-cap: 'Example plots created using `aggplot`. Left: using median, standard deviation,
#|   and coloured lines. Right: using mean, standard error, and greyscale'
...
#| fig-height: 6
#| out-width: 100%
#| fig-cap: Raw images of our butterflies
...
Images often require some post-capture processing, depending on the intended use, and `pavo` offers some useful options via `procimg()`. The functionality of `procimg()` is currently limited to only that which is closely relevant to the analyses currently implemented, as we assume any major processing and quality-checking (e.g. image rotation and colour correction) is undertaken by the user beforehand. However, should users wish to keep their image processing within the R ecosystem, there now exist several excellent image-processing packages such as `imager` and `magick`, which `pavo` can seamlessly work with. Indeed, an `as.cimg()` method and the function `rimg2magick()` convert images from `pavo`'s preferred structure to those used by `imager` and `magick` respectively (with the reverse simply achieved via `as.rimg()`), which simplifies the transitions between these packages. Since the former is an S3 method registered on `imager::as.cimg()`, it is called as `imager::as.cimg(img)` rather than through a `pavo`-specific name.
...
#| fig-height: 6
#| out-width: 100%
#| fig-cap: Images of our butterflies after modelling for the acuity of a conspecific
#|   viewer
