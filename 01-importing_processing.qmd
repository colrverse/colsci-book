...
#### Severe artifacts across all spectra

Some lamps or spectrometers can produce severe artifacts at specific wavelengths across all spectra. When trying to smooth over such artifacts, the smoothing function may not be able to remove them, or it may distort the surrounding data. In such cases, it may be better to remove the affected wavelengths from all spectra using the `opt = "clip"` argument in `procspec()`. We will see an example of this later on the `teal` data in @sec-visualising-spectral-data.
...
## Visualizing Spectral Data {#sec-visualising-spectral-data}
...
Here, we also remove the artifact between 600 and 650 nm using the `clip` option in `procspec()`.
...
#|   reflectance
#| layout-ncol: 2
teal.norm <- procspec(teal, opt = c("min", "max", "clip"), clip_range = c(600, 650))
...
teal.sm <- procspec(teal, opt = c("smooth", "clip"), span = 0.2, clip_range = c(600, 650))
