...
```{r, fig=TRUE, include=TRUE, fig.width=8.5, fig.height=5, fig.cap="Plots of species mean reflectance curves with corresponding relative usml cone stimulations (insets).", out.width = '100%'}
...
```{r, fig=TRUE, include=TRUE, fig.width=5, fig.height=4, fig.cap="The receptor sensitivities of an idealised dichromat created using `sensmodel()`."}
...
```{r, fig=TRUE, include=TRUE, fig.align = 'center',fig.width=5, fig.height=5, fig.cap="Idealized reflectance spectra and their projection on the axes of segment classification"}
fakedata1 <- vapply(
  seq(100, 500, by = 20),
  function(x) rowSums(cbind(
      dnorm(300:700, x, 30),
...
```{r, fig=TRUE, fig.aligh = 'center', include=TRUE, fig.width=5, fig.height=5, fig.cap="Visual system of a pretend mantis shrimp with 10 cones"}
...
```{r, fig=TRUE, fig.align = 'center', include=TRUE, fig.width=6, fig.height=5, fig.cap="The k-means classified images of our butterflies, along with their identified colour palettes"}
...
```{r, fig=TRUE, fig.align = 'center', fig.cap="A cryptic lizard, along with a possible sampling grid for spectral measurement.", fig.show="hold", out.width="75%"}
