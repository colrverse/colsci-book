...
Both the estimate and its limits move downward. The interval is if anything slightly wider, since the correction is itself estimated and the bootstrap carries that uncertainty along with everything else. The correction relies on the distance arising from an inner product, so it is unavailable under quantum noise, in the `CIELAB`, `CIELCh` and `coc` spaces, and for achromatic contrast in a colourspace model, where luminance contrast is a ratio rather than a distance.
