...
In brief, the algorithm allows us to gain some insight into the spatial information which can and cannot be detected by a viewer.
...
```{r, fig=TRUE, message=FALSE, include=TRUE, fig.width=4, fig.height=4, fig.cap="Images of our butterflies after modelling for the acuity of a conspecific viewer"}

# Model the appearance of our butterflies given the reduced visual acuity of another
# animal viewer as per the AcuityView 2.0 algorithm (Caves & Johnsen 2018). Here our 
# butterfly is 60 cm away, the image width is 10 cm, and the spatial resolution 
# of the viewer is 0.2-degrees.
butterflies_acuity <- procimg(butterflies, obj_dist = 60, obj_width = 10, eye_res = 0.2)
...
Note there are some important considerations and caveats to the method. The core publication of Caves & Johnsen (2018) provides rich discussion of these points (and should of course also be cited whenever used). In brief, however, it should be remembered that:
